...
import { Star, Key, Check, ShoppingCart, Info, Clock, Zap } from "lucide-react";
...
function ShopContent() {
  ...
  const handleAddToCart = (id: string, type: "rank" | "case", name: string, price: number, color?: string) => {
    addItem({ id, type, name, price, color });
    toast.success(`${name} добавлен в корзину`, { description: `${price} ₽` });
  };

  const handleBuyNow = (id: string, type: "rank" | "case", name: string, price: number, color?: string) => {
    addItem({ id, type, name, price, color });
          ...
          <div>
            ...
            >
              ...
              <p className="text-sm" style={{ color: "#a8b4d8" }}>
                Временные ранги: 2 месяца или 4 месяца. Ранг более высокого уровня включает все команды предыдущих.
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
              {visibleTiers.map((rank) => (
                ...
                >
                  ...
                  <p className="text-xs mb-4 leading-relaxed" style={{ color: "#7b87a8" }}>{rank.rtpDesc}</p>
                  ...
                  {/* Две кнопки: Купить и В корзину */}
                  <div className="flex flex-col gap-2">
                    <button
                      onClick={() => handleBuyNow(rank.id, "rank", `${rank.nameRu} (${rank.durationLabel})`, rank.price, rank.color)}
                    ...
                    >
                      <Zap size={15} /> Купить · {rank.price} ₽
                    </button>
                    <button
                      onClick={() => handleAddToCart(rank.id, "rank", `${rank.nameRu} (${rank.durationLabel})`, rank.price, rank.color)}
                      className="w-full py-2 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all duration-200 hover:-translate-y-0.5"
                      style={{
        ...
        {activeTab === "keys" && (
          <div>
            ...
            >
              <Info size={18} style={{ color: "#6c63ff", flexShrink: 0, marginTop: 2 }} />
              <p className="text-sm" style={{ color: "#a8b4d8" }}>
                Чтобы открыть кейс, возьми ключ в руку и используй его на нужном кейсе в игре.
              ...
              {DONATE_KEYS.map((k) => (
                ...
                >
                  ...
                  <div className="flex flex-col gap-2 mt-auto">
                    <button
                      onClick={() => handleBuyNow(k.id, "case", k.name, k.price, k.color)}
                      className="w-full py-2.5 rounded-xl text-sm font-bold flex items-center justify-center gap-2 transition-all duration-200 hover:-translate-y-0.5"
                      style={{
                    ...
                    >
                      <Zap size={15} /> Купить · {k.price} ₽
                    </button>
                    <button
                      onClick={() => handleAddToCart(k.id, "case", k.name, k.price, k.color)}
                      className="w-full py-2 rounded-xl text-sm font-semibold flex items-center justify-center gap-2 transition-all duration-200 hover:-translate-y-0.5"
                      style={{
                        background: "rgba(108,99,255,0.08)",
                        border: "1px solid rgba(108,99,255,0.25)",
