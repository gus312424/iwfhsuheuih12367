...
// Sanitize string input — remove HTML tags and dangerous chars
...
export function validateMinecraftUsername(username: string): {
  ...
  if (!/^[a-zA-Z0-9_]+$/.test(cleaned))
    return {
      valid: false,
      error: "Ник может содержать только буквы, цифры и _",
    };
  return { valid: true };
}
...
export function getSecurityHeaders(): Record<string, string> {
  return {
    ...
    "Content-Security-Policy":
      "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline' fonts.googleapis.com; font-src 'self' fonts.gstatic.com; img-src 'self' data: https:; connect-src 'self'; frame-ancestors 'none';",
  };
}
