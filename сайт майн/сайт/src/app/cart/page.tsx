  ...
  if (items.length === 0) {
    return (
      <div className="min-h-screen pt-32 flex flex-col items-center justify-center text-center px-4">
        ...
        <p className="text-base mb-8" style={{ color: "#7b87a8" }}>Перейди в магазин и добавь что-нибудь</p>
        <Link href="/shop" className="btn-primary flex items-center gap-2">В магазин <ArrowRight size={18} /></Link>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen pt-28 pb-20 px-4">
      <div className="max-w-5xl mx-auto">
        <h1 className="text-3xl font-black mb-8"><span className="text-gradient">Корзина</span></h1>
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          <div className="lg:col-span-2 space-y-4">
            ...
            <div className="rounded-2xl p-6" style={{ background: "rgba(13,17,32,0.9)", border: "1px solid rgba(108,99,255,0.2)" }}>
              ...
              />
              {usernameError && <p className="text-xs mt-2" style={{ color: "#fc8181" }}>{usernameError}</p>}
              {!usernameError && username.trim().length >= 3 && (
                <p className="text-xs mt-2" style={{ color: "#68d391" }}>✓ Ник корректный — товары придут на «{username}»</p>
              )}
              <p className="text-xs mt-2" style={{ color: "#7b87a8" }}>Убедись что ник написан верно — привилегии выдаются по нику автоматически</p>
            </div>
            ...
            {items.map((item) => (
              <div
                ...
                style={{ background: "rgba(13,17,32,0.9)", border: `1px solid ${item.color ? item.color + "25" : "rgba(108,99,255,0.15)"}` }}
              >
                <div
                ...
                <div className="flex-1 min-w-0">
                  <div className="font-bold text-sm mb-0.5 truncate" style={{ color: item.color || "#e8eaf6" }}>
                    {item.type === "rank" ? `Ранг: [${item.name}]` : item.name}
                  </div>
                  <div className="text-xs" style={{ color: "#7b87a8" }}>{item.price} ₽ за штуку</div>
                </div>
                <div className="flex items-center gap-2 rounded-xl px-3 py-1.5" style={{ background: "rgba(255,255,255,0.05)" }}>
                  <button onClick={() => updateQty(item.id, item.quantity - 1)} className="w-6 h-6 flex items-center justify-center" style={{ color: "#7b87a8" }}>
                    <Minus size={12} />
                  </button>
                ...
                <button
                  ...
                  className="ml-2 p-1.5 rounded-lg transition-colors hover:text-red-400"
                  style={{ color: "#7b87a8" }}
                >
                  <Trash2 size={15} />
                </button>
            ...
            <button onClick={clearCart} className="text-xs transition-colors hover:text-red-400" style={{ color: "#7b87a8" }}>
              Очистить корзину
            </button>
          </div>
          ...
          <div>
            <div className="rounded-2xl p-6 sticky top-24" style={{ background: "rgba(13,17,32,0.9)", border: "1px solid rgba(108,99,255,0.2)" }}>
              ...
              {!username.trim() && (
                <p className="text-xs text-center mb-4" style={{ color: "#fc8181" }}>Введи ник игрока чтобы продолжить</p>
              )}
              <Link
                ...
                style={{
                  background: canCheckout ? "linear-gradient(135deg,#6c63ff,#4facfe)" : "rgba(108,99,255,0.2)",
              ...
              <div className="mt-4 space-y-2">
                {["🔒 Безопасная оплата", "⚡ Выдача после оплаты", "🛡️ Гарантия возврата"].map((text) => (
                  <div key={text} className="text-xs text-center" style={{ color: "#7b87a8" }}>{text}</div>
                ))}
              </div>
            </div>
          </div>
